...
class Actor():
    ...
    def is_centered(self, check_both = False):   
        if check_both:
            ...
            up_down = self.is_centered(False)
            return(up_down and left_right)   
        desired_direction = self.velocity_to_direction.get(self.get_desired_velocity().get_value())
    ...
    def set_coordinate(self, new_coordinate:Vector2):
        if not isinstance(new_coordinate, Vector2):
            return
    ...
    def _validate_velocity(self):
        ...
        if not target_tile:
            x, y = self.direction_to_velocity[stop]
            self._velocity.set_value(x,y)
        ...
        target_tile = self.get_target_tile(self.get_velocity())
        if self.check_wall_collision(target_tile):
            x , y = self.direction_to_velocity.get(stop)
            self._velocity.set_value(x,y)
            self._desired_velocity.set_value(x,y)
